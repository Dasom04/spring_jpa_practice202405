...
class StudentPageRepositoryTest {
    ...
    void testBasicPagination() {
        ...
        // 페이지 정보 생성
        // 페이지 번호가 제로베이스 -> 0이 1페이지
        Pageable pageable = PageRequest.of(pageNo - 1, amount, // 내림차순 으로 한 것. 오름차순이면 descending 빼기
//                 Sort.by("name").descending()); // 정렬 기분값은 필드명!! 컬럼명(x)
                Sort.by(
                        Sort.Order.desc("name"), // name은 내림차순
                        Sort.Order.asc("city") // city는 오름차순
                )
        );
